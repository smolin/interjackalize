...
class corporaClass:
    ...
    fallback_base = "https://random-words-api.kushcreates.com/api"
    ...
    def _fetch_datamuse(self, query_params, pos_filter=None, max_results=100):
        url = f"{self.datamuse_base}/words"
        params = {**query_params, "max": max_results}
        resp = requests.get(url, params=params, timeout=5)
        resp.raise_for_status()
        words = [entry["word"] for entry in resp.json()]
        if pos_filter:
            params_md = {**query_params, "md": "p", "max": max_results * 2}
            resp_md = requests.get(url, params=params_md, timeout=5)
            resp_md.raise_for_status()
            words = [
                entry["word"]
                for entry in resp_md.json()
                if pos_filter in entry.get("tags", [])
            ]
        return words
    ...
    def _fetch_with_fallback(
        ...
        fallback_lang="en",
        datamuse_params=None,
        datamuse_pos=None,
    ):
        import sys

        try:
            response = requests.get(self.base + endpoint, timeout=5)
            response.raise_for_status()
            return primary_parser(response.json())
        except Exception as e:
            if fallback_category:
                try:
                    fallback_url = f"{self.fallback_base}?language={fallback_lang}&category={fallback_category}&words=100"
                    resp = requests.get(fallback_url, timeout=5)
                    resp.raise_for_status()
                    return [entry["word"] for entry in resp.json()]
                except Exception:
                    pass
            if datamuse_params:
                try:
                    return self._fetch_datamuse(datamuse_params, datamuse_pos)
                except Exception as datamuse_error:
                    print(
                        f"WARNING: All APIs failed for {endpoint}: corpora={e}, datamuse={datamuse_error}",
                        file=sys.stderr,
                    )
    ...
    def case_adjs(self):
        return self._fetch_with_fallback(
            "words/adjs",
            lambda data: data["data"]["adjs"],
            datamuse_params={"rel_jjb": "thing"},
            datamuse_pos="adj",
        )
    ...
    def case_advs(self):
        return self._fetch_with_fallback(
            "words/adverbs",
            lambda data: data["data"]["adverbs"],
            datamuse_params={"rel_jjb": "way"},
            datamuse_pos="adv",
        )
    ...
    def case_animals(self):
        return self._fetch_with_fallback(
            "animals/common",
            lambda data: data["data"]["animals"],
            fallback_category="animals",
            fallback_lang="en",
        )
    ...
    def case_colors(self):
        return self._fetch_with_fallback(
            "colors/web_colors",
            lambda data: [entry["color"] for entry in data["data"]["colors"]],
            datamuse_params={"ml": "color"},
            datamuse_pos="n",
        )
    ...
    def case_materials(self):
        ...
        try:
            result = (
                requests.get(
                    self.base + "materials/layperson-metals", timeout=5
                ).json()["data"]["layperson metals"]
                + requests.get(self.base + "materials/gemstones", timeout=5).json()[
                    "data"
                ]["gemstones"]
                + requests.get(self.base + "materials/fabrics", timeout=5).json()[
                    "data"
                ]["fabrics"]
                + requests.get(
                    self.base + "materials/building-materials", timeout=5
                ).json()["data"]["building materials"]
                + requests.get(
                    self.base + "materials/natural-materials", timeout=5
                ).json()["data"]["natural materials"]
            )
            return result
        except Exception as e:
            print(f"WARNING: materials API failed: {e}", file=sys.stderr)
            try:
                return self._fetch_datamuse({"ml": "material"})
            except:
                raise
    ...
    def case_moods(self):
        return self._fetch_with_fallback(
            "humans/moods",
            lambda data: data["data"]["moods"],
            datamuse_params={"ml": "emotion"},
            datamuse_pos="n",
        )
    ...
    def case_nouns(self):
        return self._fetch_with_fallback(
            "words/nouns",
            lambda data: data["data"]["nouns"],
            datamuse_params={"rel_jja": "good"},
            datamuse_pos="n",
        )
    ...
    def case_occupations(self):
        return self._fetch_with_fallback(
            "humans/occupations",
            lambda data: data["data"]["occupations"],
            datamuse_params={"ml": "profession"},
            datamuse_pos="n",
        )
    ...
    def case_preps(self):
        return self._fetch_with_fallback(
            "words/prepositions",
            lambda data: data["data"]["prepositions"],
            datamuse_params={"sp": "??????"},
            datamuse_pos="prep",
        )
    ...
    def case_prefs(self):
        return self._fetch_with_fallback(
            ...
            lambda data: [entry["part"] for entry in data["data"]["prefixes"]],
            datamuse_params={"sp": "pre*", "md": "p"},
            datamuse_pos=None,
        )
    ...
    def case_roots(self):
        return self._fetch_with_fallback(
            "words/prefix_root_suffix",
            lambda data: [entry["part"] for entry in data["data"]["roots"]],
            datamuse_params={"sp": "???*", "md": "p"},
            datamuse_pos=None,
        )
    ...
    def case_roles(self):
        return self._fetch_with_fallback(
            ...
            lambda data: [
                ...
                for entry in data["data"]["characters"]
            ],
            datamuse_params={"ml": "character archetype"},
            datamuse_pos="n",
        )
    ...
    def case_sufs(self):
        return self._fetch_with_fallback(
            "words/prefix_root_suffix",
            lambda data: [entry["part"] for entry in data["data"]["suffixes"]],
            datamuse_params={"sp": "*ment"},
            datamuse_pos=None,
        )
    ...
    def case_verbs(self):
        return self._fetch_with_fallback(
            ...
            datamuse_params={"rel_jja": "action"},
            datamuse_pos="v",
        )
...
class replaceMapClass:
    ...
    def case_jabberwock(self):
        return random.choice(
            "bandersnatch beamish borogoves brillig callay callooh frabjous frumious galumphing gimble gyre jabberwock jubjub manxome mimsy mome outgrabe raths slithy snicker-snack toves tulgey tumtum uffish vorpal whiffling".split(
                " "
            )
        )
    ...
    def case_latlong(self):
        return (
            str(round(random.random() * 180, 7))
            + random.choice(["W", "E"])
            + ","
            + str(round(random.random() * 90, 7))
            + random.choice(["N", "S"])
        )
    ...
    def case_rm_water(self):
        return random.choice(
            ["parched", "bone-dry", "dusty", "damp", "wet", "dripping"]
        )
...
pat = re.compile(
    "([{]([0-9 ]*)([^{}]+)[}])"
)  # eg "have a {2 adj} {noun} today": mg0 is "{2 adj}", mg1 is "2 " mg2 is "adj"
